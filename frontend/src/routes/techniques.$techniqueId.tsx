  ...
  return (
    <section className="space-y-4">
      ...
      <div className="space-y-2 rounded-lg border border-surface-600/70 bg-surface-800/70 p-3">
        ...
        <InfoPair
          label="Generated by"
          value={formatRuleProvider(data.rule_provider)}
        />
        <InfoPair
          label="Provider trace"
          value={
            data.rule_provider_trace?.length
              ? data.rule_provider_trace.join(" → ")
              : "N/A"
          }
        />
...
function formatRuleProvider(provider: string | null) {
  if (!provider) return "N/A"
  if (provider === "splunk_ai_assistant_mcp") {
    return "Splunk AI Assistant (via MCP)"
  }
  if (provider === "gemini") {
