...
def _make_agent(splunk_client=None) -> GapAgent:
    ...
    with patch.dict(
        os.environ,
        {
            ...
            "SPLUNK_AI_PRIMARY": "false",
            "SPLUNK_MCP_TOKEN": "",
        },
    ):
        with patch("agents.gap_agent.genai.Client"):
            with patch("agents.gap_agent.time.sleep"):
                return GapAgent(splunk_client or _mock_splunk(), ai_primary=False)
...
class TestSPLRuleSchema(unittest.TestCase):
    ...
    def test_string_number_confidence_is_coerced_to_float(self):
        rule = SPLRule(
            rule="search index=*", explanation="x", confidence="0.85"
        )  # passed as string
...
class TestGapAgentInit(unittest.TestCase):
    def test_raises_without_api_key(self):
        env = {
            ...
            if k not in {"GEMINI_API_KEY", "SPLUNK_MCP_TOKEN"}
        }
        env["SPLUNK_AI_PRIMARY"] = "true"
        with patch.dict(os.environ, env, clear=True):
            with self.assertRaises(ValueError):
                with patch("agents.gap_agent.genai.Client"):
                    mock_mcp = MagicMock()
                    mock_mcp.is_configured.return_value = False
                    GapAgent(_mock_splunk(), mcp_client=mock_mcp)
...
class TestGapAgentRun(unittest.TestCase):
    ...
    def test_limit_parameter_restricts_processing(self):
        ...
        processed = [
            t for t in state.techniques.values() if t.generated_rule is not None
        ]
    ...
    def test_successful_rule_staged_for_approval(self):
        ...
        agent = _make_agent(
            _mock_splunk(
                valid=True, normalized="search index=main sourcetype=proxy phishing"
            )
        )
        with patch.object(
            agent,
            "_call_ai",
            return_value=_make_spl_rule(
        ...
        self.assertEqual(
            t.generated_rule, "search index=main sourcetype=proxy phishing"
        )
...
class TestCallAi(unittest.TestCase):
    ...
    def test_returns_spl_rule_on_valid_response(self):
        ...
        mock_response = MagicMock()
        mock_response.text = (
            '{"rule": "search index=*", "explanation": "test", "confidence": 0.8}'
        )
        agent.ai.models.generate_content = MagicMock(return_value=mock_response)

        result = agent._call_ai(
            {
                "technique_id": "T1566",
                "technique_name": "Phishing",
                "tactics": [],
...
class TestBuildPrompt(unittest.TestCase):
    ...
    def test_correction_section_separated_from_main_prompt(self):
        ...
        self.assertIn(
            "\n\n", prompt, "Correction section must be separated by a blank line"
        )
